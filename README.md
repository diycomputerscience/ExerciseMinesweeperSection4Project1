...
<h2>Overview</h2>
In previous activities, we have created ```Board``` and ```Square``` classes. We also added basic functionality to these classes. Now it's time to move ahead one step and introduce a simple UI for the Minesweeper game. Keep in mind, this is not the final UI. Just something to get started with.
...
However, before proceeding, we would like to draw your attention to a few things:
 ...
 1. Notice this class has a ```main``` method. When a class i executed, it's main method is where out code gets control. We will run this class when we want to run the game.
 ...
 1. Notice the UI class has a static method called ```buildUI()```. This is more of a builder method in which you will initialize the UI. You may not be able to understand this right away, but we have introdiced this method to make it easier to write unit tests for the UI.
...
<h2>Learning Outcomes</h2>
 1. Understand that the ```main``` method of the class that is executed, is the point of entry for a program
